...
export function TravelStoriesBar() {
  const user = useAuthStore((state) => state.user) as { user_id?: number; id?: number; name?: string } | null;
  ...
  useEffect(() => {
    ...
    if (storeId) { setSessionUserId(storeId); return; }
    const token = localStorage.getItem("travel360_token") ?? localStorage.getItem("token");
    if (!token) { setSessionUserId(0); return; }
    try {
      const stored = JSON.parse(localStorage.getItem("user") ?? "{}") as { user_id?: number; id?: number };
      const storedId = Number(stored.user_id ?? stored.id ?? 0);
      if (storedId) { setSessionUserId(storedId); return; }
  ...
  async function load() {
    ...
    try { setStories(await travelStoryService.feed({ page: 1, limit: 100 })); } catch { setStories([]); } finally { setLoading(false); }
  }
  ...
  return <>
    <section className="mb-6 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between"><div><h2 className="font-bold text-ink">Travel Stories</h2><p className="text-xs text-slate-500">Photos and videos shared in the last 24 hours.</p></div><button type="button" onClick={() => setCreating(true)} className="inline-flex h-9 items-center gap-2 rounded-lg bg-brand-600 px-3 text-sm font-bold text-white"><Plus size={16} />Add story</button></div>
      <div className="mt-4 flex gap-4 overflow-x-auto pb-2">
        ...
        {!loading && !groups.length ? <p className="py-4 text-sm text-slate-500">No active stories yet. Be the first to share one.</p> : null}
      </div>
    </section>
    {creating ? <CreateStoryModal onClose={() => setCreating(false)} onCreated={() => { setCreating(false); void load(); }} /> : null}
    {activeIndex !== null && orderedStories[activeIndex] ? <StoryViewer stories={orderedStories} index={activeIndex} currentUserId={currentUserId} onIndex={setActiveIndex} onClose={() => setActiveIndex(null)} onDeleted={() => { setActiveIndex(null); void load(); }} onViewed={(id) => setStories((current) => current.map((story) => getTravelStoryId(story) === id ? { ...story, is_viewed: true } : story))} /> : null}
  </>;
}
...
function StoryViewer({ stories, index, currentUserId, onIndex, onClose, onViewed, onDeleted }: { stories: TravelStory[]; index: number; currentUserId: number; onIndex: (index: number | null) => void; onClose: () => void; onViewed: (id: number) => void; onDeleted: () => void }) {
  ...
  useEffect(() => { if (getTravelStoryUserId(story) !== currentUserId) { void travelStoryService.markViewed(id).then(() => onViewed(id)).catch(() => undefined); } timer.current = window.setTimeout(() => index < stories.length - 1 ? onIndex(index + 1) : onClose(), duration); return () => { if (timer.current) window.clearTimeout(timer.current); }; }, [id, index]);
