...
const initial: Booking[] = ([
  { id: 1, code: "BK-2048", customer: "Sophie Martin", phone: "0901234567", tour: "Saigon One Day Tour", adults: 2, children: 1, infants: 0, travelDate: "2026-06-18", status: "confirmed", ...tourCatalog["Saigon One Day Tour"] },
  { id: 2, code: "BK-2052", customer: "David Lee", phone: "0912345678", tour: "Cu Chi Heritage Route", adults: 2, children: 0, infants: 0, travelDate: "2026-07-04", status: "pending", ...tourCatalog["Cu Chi Heritage Route"] },
  { id: 3, code: "BK-2054", customer: "Emma Johnson", phone: "0923456789", tour: "Ha Long Weekend", adults: 2, children: 2, infants: 0, travelDate: "2026-07-12", status: "confirmed", ...tourCatalog["Ha Long Weekend"] },
  { id: 4, code: "BK-2055", customer: "Michael Brown", phone: "0934567890", tour: "City Museum Route", adults: 1, children: 0, infants: 0, travelDate: "2026-07-20", status: "pending", ...tourCatalog["City Museum Route"] },
  { id: 5, code: "BK-1988", customer: "Anna Wilson", phone: "0945678901", tour: "Central Vietnam Discovery", adults: 2, children: 0, infants: 1, travelDate: "2026-04-21", status: "cancelled", ...tourCatalog["Central Vietnam Discovery"] },
  { id: 6, code: "BK-2060", customer: "Minh Nguyen", phone: "0956789012", tour: "Market Food Walk", adults: 1, children: 0, infants: 0, travelDate: "2026-08-01", status: "expired", ...tourCatalog["Market Food Walk"] }
...
function BookingModal({ item, onClose, onSave }: { item: Booking; onClose: () => void; onSave: (item: Pick<Booking, "id" | "customer" | "phone" | "status">) => void }) {
  ...
  function submit() {
    ...
    onSave({ id: form.id, customer, phone, status: statusLocked ? item.status : form.status });
  }

  return <Modal title="Edit Booking" onClose={onClose} onSubmit={submit} hideSave={travelDatePassed}>
    <div className="grid gap-4 sm:grid-cols-2">
      <Field label="Customer"><input value={form.customer} onChange={(e) => { setForm({ ...form, customer: e.target.value }); setFieldErrors((current) => ({ ...current, customer: "" })); }} className={`input ${fieldErrors.customer ? "border-rose-500" : ""}`} />{fieldErrors.customer ? <span className="mt-2 block text-xs font-semibold text-rose-600">{fieldErrors.customer}</span> : null}</Field>
      <Field label="Phone"><input type="tel" value={form.phone} onChange={(e) => { setForm({ ...form, phone: e.target.value }); setFieldErrors((current) => ({ ...current, phone: "" })); }} className={`input ${fieldErrors.phone ? "border-rose-500" : ""}`} />{fieldErrors.phone ? <span className="mt-2 block text-xs font-semibold text-rose-600">{fieldErrors.phone}</span> : null}</Field>
      <Field label="Tour"><input value={item.tour} readOnly disabled className="input bg-slate-50 text-slate-500" /></Field>
      <Field label="Travel Date"><input value={item.travelDate} readOnly disabled className="input bg-slate-50 text-slate-500" /></Field>
      <Field label="Status"><select value={form.status} disabled={statusLocked} onChange={(e) => setForm({ ...form, status: e.target.value as BookingStatus })} className="input disabled:bg-slate-50 disabled:text-slate-500">{getStatusOptions(item.status).map((status) => <option key={status} value={status}>{status}</option>)}</select>{statusLocked ? <span className="mt-2 block text-xs font-semibold text-slate-500">Cancelled bookings cannot change status.</span> : null}</Field>
      <ReadOnlyMetric label="Adults" value={item.adults} />
      <ReadOnlyMetric label="Children" value={item.children} />
      <ReadOnlyMetric label="Infants" value={item.infants} />
      <AmountPreview amount={item.amount} />
    </div>
    <div className="mt-4 flex flex-col gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-600 sm:flex-row sm:items-center sm:justify-between">
      <span><span className="font-semibold text-slate-800">Remaining Seats:</span> {item.remainingSeats}</span>
...
function Modal({ title, children, onClose, onSubmit, saveDisabled, hideSave }: { title: string; children: React.ReactNode; onClose: () => void; onSubmit: () => void; saveDisabled?: boolean; hideSave?: boolean }) { return <div className="fixed inset-0 z-50 grid place-items-center bg-black/45 p-4"><form className="max-h-[90vh] w-full max-w-2xl overflow-auto rounded-lg border border-slate-200 bg-white p-6 shadow-soft" onSubmit={(e) => { e.preventDefault(); onSubmit(); }}><h2 className="text-xl font-bold">{title}</h2><div className="mt-6">{children}</div><div className="mt-6 flex justify-end gap-3"><Button type="button" variant="outline" onClick={onClose}>Cancel</Button>{hideSave ? null : <Button type="submit" disabled={saveDisabled}>Save</Button>}</div></form></div>; }
...
function getStatusOptions(status: BookingStatus) {
  return status === "cancelled" ? ["cancelled"] as BookingStatus[] : ["pending", "confirmed", "completed", "cancelled", "expired"] as BookingStatus[];
}
