...
const initialBookings: ManagedBooking[] = ([
  { id: "BK-2048", user: "Sophie Martin", phone: "0901234567", tour: "Saigon One Day Tour", travelDate: "2026-06-18", adults: 2, children: 1, infants: 0, status: "Confirmed", ...tourCatalog["Saigon One Day Tour"] },
  { id: "BK-2052", user: "David Lee", phone: "0912345678", tour: "Historic Ho Chi Minh City Tour", travelDate: "2026-07-04", adults: 1, children: 0, infants: 0, status: "Pending", ...tourCatalog["Historic Ho Chi Minh City Tour"] },
  { id: "BK-2054", user: "Emma Johnson", phone: "0923456789", tour: "Ha Long Bay Weekend", travelDate: "2026-07-12", adults: 2, children: 2, infants: 0, status: "Confirmed", ...tourCatalog["Ha Long Bay Weekend"] },
  { id: "BK-2055", user: "Michael Brown", phone: "0934567890", tour: "Saigon One Day Tour", travelDate: "2026-07-20", adults: 1, children: 0, infants: 0, status: "Confirmed", ...tourCatalog["Saigon One Day Tour"] },
  { id: "BK-2056", user: "Minh Nguyen", phone: "0945678901", tour: "Cu Chi Heritage Route", travelDate: "2026-08-02", adults: 3, children: 0, infants: 0, status: "Pending", ...tourCatalog["Cu Chi Heritage Route"] },
  { id: "BK-1988", user: "Anna Wilson", phone: "0956789012", tour: "Central Vietnam Discovery", travelDate: "2026-04-21", adults: 2, children: 0, infants: 1, status: "Cancelled", ...tourCatalog["Central Vietnam Discovery"] }
] satisfies Array<Omit<ManagedBooking, "amount">>).map((booking) => ({ ...booking, amount: calculateAmount(booking) }));
const emptyBooking: ManagedBooking = { id: "", user: "", phone: "", tour: "Saigon One Day Tour", travelDate: "", adults: 1, children: 0, infants: 0, amount: 150, status: "Pending", ...tourCatalog["Saigon One Day Tour"] };
...
export default function AdminBookingsPage() {
  ...
  const pageSize = 5;
  const visibleItems = items.filter((item) => `${item.id} ${item.user} ${item.phone} ${item.tour} ${item.status}`.toLowerCase().includes(query.toLowerCase()));
  ...
  function save(payload: ManagedBooking) {
    setItems((current) => editing
      ? current.map((item) => item.id === editing.id ? { ...item, user: payload.user, phone: payload.phone, status: payload.status } : item)
      : [...current, { ...payload, amount: calculateAmount(payload), id: `BK-${Math.max(...current.map((item) => Number(item.id.replace("BK-", ""))), 2000) + 1}` }]);
    setEditing(null);
    setCreating(false);
  }
  ...
  return <>
    <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-6 overflow-x-auto"><table className="w-full min-w-[1080px] text-left text-sm"><thead className="bg-slate-50 text-slate-500"><tr>{["Booking ID", "User", "Phone", "Tour", "Travel Date", "Passengers", "Amount", "Status", "Actions"].map((heading) => <th key={heading} className="p-3">{heading}</th>)}</tr></thead><tbody>
        {paginatedItems.map((item) => <tr key={item.id} className="border-t border-slate-100"><td className="p-3 font-bold"><CalendarCheck className="mr-2 inline size-4 text-brand-600" />{item.id}</td><td className="p-3">{item.user}</td><td className="p-3">{item.phone}</td><td className="p-3 text-slate-600">{item.tour}</td><td className="p-3">{item.travelDate}</td><td className="p-3">{item.adults} adult, {item.children} child, {item.infants} infant</td><td className="p-3 font-semibold">{currency(item.amount)}</td><td className="p-3"><Status value={item.status} /></td><td className="p-3"><Button variant="outline" className="h-9 px-3" onClick={() => setEditing(item)}><Pencil size={15} /> Edit</Button></td></tr>)}
      </tbody></table></div>
...
function BookingForm({ title, initialValue, isEditing, onClose, onSave }: { title: string; initialValue: ManagedBooking; isEditing: boolean; onClose: () => void; onSave: (payload: ManagedBooking) => void }) {
  ...
  const editable = !travelDatePassed;
  ...
  const statusOptions = isEditing ? getStatusOptions(initialValue.status) : ["Pending", "Confirmed", "Cancelled"] as BookingStatus[];
  ...
  function submit() {
    setMessage("");
    const errors = validateContact(form.user, form.phone);
    ...
    onSave({
      ...
      amount: isEditing ? initialValue.amount : amount,
      remainingSeats: isEditing ? initialValue.remainingSeats : seatsAfterEdit,
      status: statusLocked ? initialValue.status : form.status
    });
  }
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-black/45 p-4"><form className="max-h-[90vh] w-full max-w-2xl overflow-auto rounded-lg border border-slate-200 bg-white p-6 shadow-soft" onSubmit={(event) => { event.preventDefault(); submit(); }}>
    ...
    <div className="mt-6 grid gap-4 sm:grid-cols-2">
      <Field label="User"><input value={form.user} disabled={travelDatePassed} onChange={(event) => { setForm({ ...form, user: event.target.value }); setFieldErrors((current) => ({ ...current, user: "" })); }} className={`input disabled:bg-slate-50 disabled:text-slate-500 ${fieldErrors.user ? "border-rose-500" : ""}`} placeholder="Sophie Martin" />{fieldErrors.user ? <span className="mt-2 block text-xs font-semibold text-rose-600">{fieldErrors.user}</span> : null}</Field>
      <Field label="Phone"><input type="tel" value={form.phone} disabled={travelDatePassed} onChange={(event) => { setForm({ ...form, phone: event.target.value }); setFieldErrors((current) => ({ ...current, phone: "" })); }} className={`input disabled:bg-slate-50 disabled:text-slate-500 ${fieldErrors.phone ? "border-rose-500" : ""}`} placeholder="0901234567" />{fieldErrors.phone ? <span className="mt-2 block text-xs font-semibold text-rose-600">{fieldErrors.phone}</span> : null}</Field>
      ...
      <Field label="Status"><select value={form.status} disabled={!editable || statusLocked} onChange={(event) => setForm({ ...form, status: event.target.value as BookingStatus })} className="input disabled:bg-slate-50 disabled:text-slate-500">{statusOptions.map((status) => <option key={status} value={status}>{status}</option>)}</select>{statusLocked ? <span className="mt-2 block text-xs font-semibold text-slate-500">Cancelled bookings cannot change status.</span> : null}</Field>
      {isEditing ? <ReadOnlyMetric label="Adults" value={form.adults} /> : <PassengerStepper label="Adults" value={form.adults} minusDisabled={!editable || form.adults <= 1} plusDisabled={!editable || seatLimitReached} onMinus={() => updatePassenger("adults", -1)} onPlus={() => updatePassenger("adults", 1)} />}
      {isEditing ? <ReadOnlyMetric label="Children" value={form.children} /> : <PassengerStepper label="Children" value={form.children} minusDisabled={!editable || form.children <= 0} plusDisabled={!editable || seatLimitReached} onMinus={() => updatePassenger("children", -1)} onPlus={() => updatePassenger("children", 1)} />}
      {isEditing ? <ReadOnlyMetric label="Infants" value={form.infants} /> : <PassengerStepper label="Infants" value={form.infants} minusDisabled={!editable || form.infants <= 0} plusDisabled={!editable || seatLimitReached} onMinus={() => updatePassenger("infants", -1)} onPlus={() => updatePassenger("infants", 1)} />}
      <AmountPreview amount={isEditing ? initialValue.amount : amount} />
    </div>
    <div className="mt-4 flex flex-col gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm text-slate-600 sm:flex-row sm:items-center sm:justify-between">
      <RemainingSeats value={isEditing ? initialValue.remainingSeats : seatsAfterEdit} />
    ...
    {hasSeatError && !isEditing ? <p className="mt-4 rounded-lg border border-rose-200 bg-rose-50 p-3 text-sm font-semibold text-rose-700">Not enough seats available. Only {editableSeatCapacity} seats available for this booking.</p> : null}
    ...
    <div className="mt-6 flex justify-end gap-3"><Button type="button" variant="outline" onClick={onClose}>Cancel</Button>{travelDatePassed ? null : <Button type="submit" disabled={!editable || (!isEditing && hasSeatError)}>Save Booking</Button>}</div>
  </form></div>;
}
...
function ReadOnlyMetric({ label, value }: { label: string; value: number }) { return <div className="block text-sm font-semibold"><span>{label}</span><div className="mt-2 grid h-11 place-items-center rounded-lg border border-slate-200 bg-slate-50 text-sm font-bold text-slate-700">{value}</div></div>; }
...
function getStatusOptions(status: BookingStatus) {
  return status === "Cancelled" ? ["Cancelled"] as BookingStatus[] : ["Pending", "Confirmed", "Completed", "Cancelled"] as BookingStatus[];
}
function validateContact(user: string, phone: string) {
  const errors: Record<string, string> = {};
  const customer = user.trim();
  const phoneNumber = phone.trim();

  if (!customer) errors.user = "Customer name is required.";
  else if (customer.length < 2) errors.user = "Customer name must contain at least 2 characters.";
  else if (customer.length > 100) errors.user = "Customer name must contain at most 100 characters.";
  else if (!/^[\p{L}\s]+$/u.test(customer)) errors.user = "Customer name must not contain special characters.";

  if (!/^\d{10}$/.test(phoneNumber)) errors.phone = "Phone number must contain exactly 10 digits.";
