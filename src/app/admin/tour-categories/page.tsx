...
import {
  ...
  type AdminTourCategoryPayload
} from "@/services/admin-tour-category.service";

function normalizeCategories(items: AdminTourCategory[]) {
...
export default function TourCategoriesPage() {
  ...
  async function loadCategories() {
    ...
    setError("");
    try {
      const data = await adminTourCategoryService.list();
      setItems(normalizeCategories(Array.isArray(data) ? data : []));
    } catch (err) {
      setError("Cannot load tour categories from API.");
      showToast({ variant: "error", title: "Load failed", description: "Cannot load tour categories from API." });
    } finally {
      setLoading(false);
    }
  }
  ...
  async function saveCategory(payload: AdminTourCategoryPayload) {
    ...
    try {
      if (editing) {
        ...
        await adminTourCategoryService.create(payload);
  ...
  async function deleteCategory() {
    ...
    setError("");
    try {
      await adminTourCategoryService.remove(getTourCategoryId(deleting));
      showToast({ variant: "success", title: "Category deleted", description: deleting.name });
      setDeleting(null);
      await loadCategories();
    } catch (err) {
      setError("Cannot delete tour category. It may still be used by tours.");
      showToast({ variant: "error", title: "Delete failed", description: "This category may still be used by tours." });
    } finally {
      setSaving(false);
    }
  }
    ...
    <>
      <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-2xl font-bold">TourCategory Management</h1>
            <p className="mt-1 text-sm text-slate-500">Create and update categories used to classify bookable tours.</p>
          </div>
          ...
          <table className="w-full min-w-[720px] text-left text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={getTourCategoryId(item)} className="border-t border-slate-100">
                  <td className="p-3 font-bold">#{getTourCategoryId(item)}</td>
