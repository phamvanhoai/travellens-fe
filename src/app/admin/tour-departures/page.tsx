...
export default function TourDeparturesPage() {
  ...
  async function searchTours(page: number, append: boolean) {
    ...
    try {
      ...
      if (sequence === searchSequence.current) setTourSearchLoading(false);
    }
  }
  ...
  return <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
    ...
    <div className="mt-6 grid gap-3 md:grid-cols-[minmax(220px,1fr)_180px_170px_170px_auto]"><div className="relative"><Search className="absolute left-3 top-3.5 text-slate-400" size={16} /><input value={departureSearch} onChange={(event) => { setDepartureSearch(event.target.value); setPage(1); }} placeholder="Search date or status..." className="h-11 w-full rounded-lg border border-slate-200 pl-9 pr-3 text-sm outline-none focus:border-brand-600" /></div><select value={statusFilter} onChange={(event) => { setStatusFilter(event.target.value); setPage(1); }} className="h-11 rounded-lg border border-slate-200 px-3 text-sm"><option value="">All statuses</option>{["draft", "open", "closed", "sold_out", "cancelled", "departed"].map((status) => <option key={status}>{status}</option>)}</select><input aria-label="From date" title="From date" type="date" value={dateFrom} onChange={(event) => { setDateFrom(event.target.value); setPage(1); }} className="h-11 rounded-lg border border-slate-200 px-3 text-sm" /><input aria-label="To date" title="To date" type="date" min={dateFrom} value={dateTo} onChange={(event) => { setDateTo(event.target.value); setPage(1); }} className="h-11 rounded-lg border border-slate-200 px-3 text-sm" /><Button type="button" variant="outline" className="h-11 px-4" onClick={() => { setDepartureSearch(""); setStatusFilter(""); setDateFrom(""); setDateTo(""); setPage(1); }}>Clear</Button></div>
    <div className="mt-6 overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-slate-50 text-slate-500"><tr>{["Departure", "Sale window", "Capacity", "Available", "Adult / Child", "Status", "Actions"].map((title) => <th key={title} className="p-3">{title}</th>)}</tr></thead><tbody>{loading ? <AdminTableSkeleton columns={7} rows={10} /> : items.length ? items.map((item) => <tr key={item.tour_departure_id} className="h-16 border-t border-slate-100"><td className="p-3 font-semibold">{dateTime(item.departure_at)}</td><td className="p-3 text-xs text-slate-500">{item.booking_open_at ? dateTime(item.booking_open_at) : "Now"}<br />to {item.booking_close_at ? dateTime(item.booking_close_at) : "Default cutoff"}</td><td className="p-3">{item.capacity}</td><td className="p-3 font-bold text-brand-700">{item.available_slots}</td><td className="p-3">{money(item.price)} / {money(item.child_price)}</td><td className="p-3"><span className={`rounded-full px-3 py-1 text-xs font-bold ${item.status === "open" ? "bg-emerald-50 text-emerald-700" : item.status === "cancelled" ? "bg-rose-50 text-rose-700" : "bg-slate-100 text-slate-600"}`}>{item.status}</span></td><td className="p-3"><div className="flex gap-2"><button onClick={() => openEdit(item)} className="grid size-9 place-items-center rounded-lg border border-slate-200"><Pencil size={15} /></button><button onClick={() => void remove(item)} className="grid size-9 place-items-center rounded-lg border border-rose-200 text-rose-600"><Trash2 size={15} /></button></div></td></tr>) : <tr><td colSpan={7} className="h-64 p-10 text-center text-slate-500">No departures match the current filters.</td></tr>}</tbody></table></div>
    {!loading ? <Pagination page={page} pageCount={pageCount} totalItems={totalItems} pageSize={10} itemLabel="departures" onPageChange={setPage} /> : null}
