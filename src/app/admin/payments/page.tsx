...
const initialPayments: ManagedPayment[] = [
  { id: "PAY-3001", bookingId: "BK-2048", amount: 387, currencyCode: "USD", method: "Visa", transactionCode: "TXN-827391", paymentStatus: "Paid", refundStatus: "None" },
  { id: "PAY-3002", bookingId: "BK-2052", amount: 398, currencyCode: "USD", method: "PayPal", transactionCode: "TXN-827402", paymentStatus: "Pending", refundStatus: "None" },
  { id: "PAY-3003", bookingId: "BK-2054", amount: 620, currencyCode: "USD", method: "Mastercard", transactionCode: "TXN-827455", paymentStatus: "Paid", refundStatus: "None" },
  { id: "PAY-3004", bookingId: "BK-2055", amount: 129, currencyCode: "USD", method: "Apple Pay", transactionCode: "TXN-827461", paymentStatus: "Paid", refundStatus: "None" },
  { id: "PAY-3005", bookingId: "BK-2056", amount: 420, currencyCode: "USD", method: "Visa", transactionCode: "TXN-827498", paymentStatus: "Failed", refundStatus: "None" },
  { id: "PAY-2988", bookingId: "BK-1988", amount: 356, currencyCode: "USD", method: "Visa", transactionCode: "TXN-826910", paymentStatus: "Refunded", refundStatus: "Completed" }
];
const emptyPayment: ManagedPayment = { id: "", bookingId: "", amount: 0, currencyCode: "USD", method: "Visa", transactionCode: "", paymentStatus: "Pending", refundStatus: "None" };
  ...
  return <>
    <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-6 overflow-x-auto"><table className="w-full min-w-[1040px] text-left text-sm"><thead className="bg-slate-50 text-slate-500"><tr>{["Payment ID", "Booking ID", "Amount", "Method", "Transaction Code", "Payment Status", "Refund Status", "Actions"].map((heading) => <th key={heading} className="p-3">{heading}</th>)}</tr></thead><tbody>
        {paginatedItems.map((item) => <tr key={item.id} className="border-t border-slate-100"><td className="p-3 font-bold"><CreditCard className="mr-2 inline size-4 text-brand-600" />{item.id}</td><td className="p-3 font-semibold">{item.bookingId}</td><td className="p-3">{currency(item.amount)} <span className="text-xs text-slate-500">{item.currencyCode}</span></td><td className="p-3">{item.method}</td><td className="p-3 text-slate-600">{item.transactionCode}</td><td className="p-3"><PaymentBadge value={item.paymentStatus} /></td><td className="p-3"><RefundBadge value={item.refundStatus} /></td><td className="p-3"><Button variant="outline" className="h-9 px-3" onClick={() => setEditing(item)}><Pencil size={15} /> Edit</Button></td></tr>)}
      </tbody></table></div>
...
function PaymentForm({ title, initialValue, onClose, onSave }: { title: string; initialValue: ManagedPayment; onClose: () => void; onSave: (payload: ManagedPayment) => void }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-black/45 p-4"><form className="max-h-[90vh] w-full max-w-xl overflow-auto rounded-lg border border-slate-200 bg-white p-6 shadow-soft" onSubmit={(event) => { event.preventDefault(); onSave(form); }}>
    ...
    <div className="mt-6 grid gap-4 sm:grid-cols-2">
      ...
      <Field label="Currency"><select value={form.currencyCode} onChange={(event) => setForm({ ...form, currencyCode: event.target.value })} className="input"><option>USD</option><option>VND</option><option>EUR</option></select></Field>
