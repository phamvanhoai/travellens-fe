...
import { Bold, Italic, List, Loader2, Newspaper, Pencil, Plus, RefreshCw, Trash2, X } from "lucide-react";
import { ConfirmDialog } from "@/components/common/confirm-dialog";
import { useToast } from "@/components/common/toast";
import { Button } from "@/components/ui/button";
import { authService } from "@/services/auth.service";
import { blogService, getCustomerBlogId, getCustomerBlogLocationIds, getCustomerBlogUserId, type CustomerBlog } from "@/services/blog.service";
...
export default function CustomerBlogsPage() {
  ...
  const [editing, setEditing] = useState<CustomerBlog | null>(null);
  const [deleting, setDeleting] = useState<CustomerBlog | null>(null);
  ...
  async function saveStory(form: StoryForm) {
    setSaving(true);
    setError("");
    try {
      const payload = { user_id: userId, title: form.title.trim(), content: form.content, location_ids: form.location_ids.map(Number) };
      if (editing) {
        await blogService.update(getCustomerBlogId(editing), payload);
        showToast({ variant: "success", title: "Story updated", description: form.title });
      } else {
  ...
  async function deleteStory() {
    ...
    setError("");
    try {
      await blogService.remove(getCustomerBlogId(deleting));
      setItems((current) => current.filter((blog) => getCustomerBlogId(blog) !== getCustomerBlogId(deleting)));
      showToast({ variant: "success", title: "Story deleted", description: deleting.title });
      setDeleting(null);
    } catch (err) {
      const message = getApiError(err, "Cannot delete this travel story.");
      setError(message);
  ...
  return <>
    <div className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">{loading ? <div className="col-span-full p-10 text-center text-slate-500"><Loader2 className="mr-2 inline size-5 animate-spin" /> Loading stories...</div> : items.length === 0 ? <div className="col-span-full rounded-lg bg-slate-50 p-10 text-center text-slate-500">You have not written any travel stories yet.</div> : items.map((blog) => <article key={getCustomerBlogId(blog)} className="rounded-lg border border-slate-200 p-5"><Newspaper className="size-6 text-brand-600" /><h2 className="mt-3 text-lg font-bold">{blog.title}</h2><p className="mt-2 line-clamp-3 text-sm leading-6 text-slate-600">{plainText(blog.content)}</p><div className="mt-4 flex items-center justify-between gap-3"><p className="text-xs font-semibold text-slate-400">{blog.status ?? "Submitted"}</p><span className="flex gap-2"><Button type="button" variant="outline" className="h-9 px-3" onClick={() => setEditing(blog)}><Pencil size={15} /> Edit</Button><button type="button" onClick={() => setDeleting(blog)} className="grid size-9 place-items-center rounded-lg border border-rose-200 text-rose-600 hover:bg-rose-50" aria-label={`Delete ${blog.title}`}><Trash2 size={15} /></button></span></div></article>)}</div>
    </div>
    {creating || editing ? <StoryModal key={editing ? getCustomerBlogId(editing) : "create"} initialValue={editing ? toStoryForm(editing) : emptyForm} title={editing ? "Edit Travel Story" : "Write Blog / Travel Story"} locations={locations} saving={saving} onClose={() => { setCreating(false); setEditing(null); }} onSave={saveStory} /> : null}
    {deleting ? <ConfirmDialog title="Delete travel story?" message={`Delete “${deleting.title}”? This action cannot be undone.`} onCancel={() => setDeleting(null)} onConfirm={() => void deleteStory()} /> : null}
  </>;
}

function StoryModal({ initialValue, title, locations, saving, onClose, onSave }: { initialValue: StoryForm; title: string; locations: PublicLocation[]; saving: boolean; onClose: () => void; onSave: (form: StoryForm) => Promise<void> }) {
  const [form, setForm] = useState(initialValue);
  const [errors, setErrors] = useState<StoryErrors>({});
  return <div className="fixed inset-0 z-50 grid place-items-center bg-black/45 p-4"><form noValidate className="max-h-[92vh] w-full max-w-2xl overflow-auto rounded-lg bg-white p-6 shadow-soft" onSubmit={(event) => { event.preventDefault(); const next = validateStory(form); setErrors(next); if (!Object.keys(next).length) void onSave(form); }}><div className="flex items-center justify-between"><div><h2 className="text-xl font-bold">{title}</h2><p className="mt-1 text-sm text-slate-500">Tell other travelers about your experience.</p></div><button type="button" onClick={onClose} disabled={saving} className="grid size-9 place-items-center rounded-full hover:bg-slate-100" aria-label="Close"><X size={18} /></button></div><div className="mt-6 grid gap-4"><Field label="Title" message={errors.title}><input value={form.title} onChange={(event) => { setForm({ ...form, title: event.target.value }); setErrors((current) => ({ ...current, title: undefined })); }} className="input" /></Field><StoryEditor value={form.content} message={errors.content} onChange={(content) => { setForm((current) => ({ ...current, content })); setErrors((current) => ({ ...current, content: undefined })); }} /><fieldset><legend className="text-sm font-semibold">Related Locations</legend><div className={`mt-2 grid max-h-48 gap-2 overflow-auto rounded-lg border p-3 sm:grid-cols-2 ${errors.location_ids ? "border-rose-500" : "border-slate-200"}`}>{locations.map((location) => { const id = String(getPublicLocationId(location)); return <label key={id} className="flex items-center gap-2 rounded-md p-2 text-sm hover:bg-slate-50"><input type="checkbox" checked={form.location_ids.includes(id)} onChange={() => { setForm((current) => ({ ...current, location_ids: current.location_ids.includes(id) ? current.location_ids.filter((item) => item !== id) : [...current.location_ids, id] })); setErrors((current) => ({ ...current, location_ids: undefined })); }} />{location.name}</label>; })}</div>{errors.location_ids ? <p className="mt-2 text-xs font-semibold text-rose-600">{errors.location_ids}</p> : null}</fieldset></div><div className="mt-6 flex justify-end gap-3"><Button type="button" variant="outline" onClick={onClose} disabled={saving}>Cancel</Button><Button type="submit" disabled={saving}>{saving ? <Loader2 className="size-4 animate-spin" /> : null} {title.startsWith("Edit") ? "Save Changes" : "Publish Story"}</Button></div></form></div>;
}

function StoryEditor({ value, message, onChange }: { value: string; message?: string; onChange: (value: string) => void }) {
  const ref = useRef<HTMLDivElement>(null);
  const initialValue = useRef(value);
  useEffect(() => { if (ref.current) ref.current.innerHTML = initialValue.current; }, []);
...
function toStoryForm(blog: CustomerBlog): StoryForm { return { title: blog.title, content: blog.content ?? "", location_ids: getCustomerBlogLocationIds(blog).map(String) }; }
